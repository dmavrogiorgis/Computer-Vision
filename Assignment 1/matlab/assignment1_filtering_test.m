...
identity_image_test =  imfilter(test_image, identity_filter);

disp('MSE between imfilter and my_imfilter:');
sum(mse(identity_image,identity_image_test))

figure(2); imshow(identity_image);
imwrite(identity_image, 'test_output/identity_image.jpg', 'quality', 95);
...
blur_image_test =  imfilter(test_image, blur_filter);

disp('MSE between imfilter and my_imfilter:');
mse(identity_image,blur_image_test)

figure(3); imshow(blur_image);
imwrite(blur_image, 'test_output/blur_image.jpg', 'quality', 95);
...
large_blur_image_test = imfilter(test_image, large_1d_blur_filter);
large_blur_image_test = imfilter(large_blur_image_test, large_1d_blur_filter');

disp('MSE between imfilter and my_imfilter:');
mse(large_blur_image,large_blur_image_test)

figure(4); imshow(large_blur_image);
imwrite(large_blur_image, 'test_output/large_blur_image.jpg', 'quality', 95);
...
sobel_image_test = imfilter(test_image, sobel_filter);

disp('MSE between imfilter and my_imfilter:');
mse(sobel_image,sobel_image_test)

%0.5 added because the output image is centered around zero otherwise and mostly black
figure(5); imshow(sobel_image + 0.5);
imwrite(sobel_image + 0.5, 'test_output/sobel_image.jpg', 'quality', 95);
...
laplacian_image_test = imfilter(test_image, laplacian_filter);

disp('MSE between imfilter and my_imfilter:');
mse(laplacian_image,laplacian_image_test)

%0.5 added because the output image is centered around zero otherwise and mostly black
figure(6); imshow(laplacian_image + 0.5);
imwrite(laplacian_image + 0.5, 'test_output/laplacian_image.jpg', 'quality', 95);

%% High pass "filter" alternative
high_pass_image = test_image - blur_image; %simply subtract the low frequency content

figure(7); imshow(high_pass_image + 0.5);
imwrite(high_pass_image + 0.5, 'test_output/high_pass_image.jpg', 'quality', 95);
